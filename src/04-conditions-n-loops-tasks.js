...
function doRectanglesOverlap(rect1, rect2) {
  ...
  // Return true if there's overlap along both axes, otherwise false
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  // Determine the correct ordering of the start and end points
  const start = Math.min(a, b);
  const end = Math.max(a, b);

  // Convert inclusion/exclusion flags into symbols
  const startSymbol = isStartIncluded ? '[' : '(';
  const endSymbol = isEndIncluded ? ']' : ')';

  // Combine symbols and points to create the interval string
...
function reverseString(str) {
  ...
  // Iterate through the string in reverse and build the reversed string
